...
function resortLink(head, nodesNum, nodes) {
  const linkNodes = [];
  ...
  while (linkNodes.length) {
    queue.push(linkNodes.pop());
    queue.push(linkNodes.shift());
  }

  // 更新指针
  for (let i = 0; i < queue.length; i++) {
    // 这里要考虑奇数个时，queue[i]可能是undefined，因为在上面的shift()时是undefined
    if (!queue[i]) break;
    queue[i].next = queue[i + 1] ? queue[i + 1].address : -1;
